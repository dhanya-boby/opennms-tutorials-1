...
For more sophisticated scenarios, OpenNMS provides an alarm correlation framework which can allow structured rules or external machine learning plugins to detect `situations` which can group together the `root cause` and `sympathetic alarms` for an incident. 
...
However, those with some programming experience can create drools rules to change the alarm lifecycle and also to correlate specific alarm types into situations. 
It should be emphasised that creating and testing these rules is not easy but is included here to illustrate what can be achieved with a bit of effort.
